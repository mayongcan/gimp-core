package com.gimplatform.core.generator.utils.sqlparse;
...
public class SqlParseHelper {
    ...
    private static final Logger logger = LogManager.getLogger(SqlParseHelper.class);
    ...
    public static class NameWithAlias {
        ...
        public String toString() {
            return StringUtils.isBlank(alias) ? name : name + " as " + alias;
        }
    }
    ...
    public static Set<NameWithAlias> getTableNamesByQuery(String sql) {
        ...
        if (m.find()) {
            ...
            if (from.indexOf(',') >= 0) {
                ...
                // join的多表
                String removedFrom = StringUtils.removeMany(from.toLowerCase(), "inner", "full", "left", "right", "outer");
    ...
    public static NameWithAlias parseSqlAlias(String str) {
        ...
        if (array.length >= 2) {
            String tmpStr[] = array[1].split(" \n\t");
            return new NameWithAlias(array[0], tmpStr[0]);
        }
        array = str.split(" \n\t");
        if (array.length >= 2) {
    ...
    public static class NamedSqlConverter {
        ...
        private String replaceInsertSql2NamedParameters(String sql) {
            ...
            if (m.find()) {
                ...
                if (columns.length != values.length) {
                    throw new IllegalArgumentException("insert 语句的插入列与值列数目不相等,sql:" + sql + " columns:" + StringUtils.join(columns, ",") + " values:" + StringUtils.join(values, ","));
                }
                for (int i = 0; i < columns.length; i++) {
                    String column = columns[i];
                    String paranName = StringUtils.changeFirstCharacterCase(StringUtils.makeAllWordFirstLetterUpperCase(column), false);
                    values[i] = values[i].replace("?", prefix + paranName + suffix);
                    ;
                }
                return replace(m.start(2), m.end(2), sql, StringUtils.join(values, ","));
            }
        ...
        private String replace2NamedParametersByOperator(String sql, String operator) {
            ...
            while (m.find()) {
                ...
                String paramName = StringUtils.changeFirstCharacterCase(StringUtils.makeAllWordFirstLetterUpperCase(columnSqlName), false);
         ...
         * @return
         */
        private String replace(int start, int end, String str, String replacement) {
    ...
    public static String getPrettySql(String sql) {
        try {
            if (FileUtils.readLines(new StringReader(sql)).size() > 1) {
                return sql;
            } else {
                return replace(replace(sql, "from", "\n\tfrom"), "where", "\n\twhere");
            }
        } catch (Exception e) {
    ...
    public static String getFromClauses(String sql) {
        ...
        if (fromEnd == -1) {
            fromEnd = StringUtils.indexOfByRegex(lowerSql, "group\\s+by");
        }
        if (fromEnd == -1) {
            fromEnd = lowerSql.indexOf("having");
        }
        if (fromEnd == -1) {
            fromEnd = StringUtils.indexOfByRegex(lowerSql, "order\\s+by");
        }
        if (fromEnd == -1) {
    ...
    public static void setRandomParamsValueForPreparedStatement(String sql, PreparedStatement ps) throws SQLException {
        int count = StringUtils.containsCount(sql, "?");
    ...
    private static void warn(String sql, int i, SQLException error) {
        logger.warn("error on set parametet index:" + i + " cause:" + error + " sql:" + sql);
    }
