	...
	public static Set<NameWithAlias> getTableNamesByQuery(String sql) {
		...
		if (m.find()) {
			...
			if (from.indexOf(',') >= 0) {
				...
				// join的多表
				String removedFrom = StringUtils.removeMany(from.toLowerCase(), "inner", "full", "left", "right",
						"outer");
		...
		private String replaceInsertSql2NamedParameters(String sql) {
			...
			if (m.find()) {
				...
				for (int i = 0; i < columns.length; i++) {
					...
					String paranName = StringUtils
							.changeFirstCharacterCase(StringUtils.makeAllWordFirstLetterUpperCase(column), false);
		...
		private String replace2NamedParametersByOperator(String sql, String operator) {
			...
			while (m.find()) {
				...
				String paramName = StringUtils
						.changeFirstCharacterCase(StringUtils.makeAllWordFirstLetterUpperCase(columnSqlName), false);
