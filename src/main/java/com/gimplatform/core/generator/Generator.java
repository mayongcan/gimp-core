...
public class Generator {
	...
	private List<Exception> scanTemplatesAndProcess(File templateRootDir, Map<String, Object> templateModel, Map<String,Object> filePathModel,
	...
	private class TemplateProcessor {
		...
		private void executeGenerate(File templateRootDir, Map<String, Object> templateModel, Map<String,Object> filePathModel, File srcFile)
		...
		private void executeDelete(File templateRootDir, Map<String, Object> templateModel, Map<String,Object> filePathModel, File srcFile)
				throws SQLException, IOException, TemplateException {
			String templateFile = FileUtils.getRelativePath(templateRootDir, srcFile);
			if (GeneratorUtils.isIgnoreTemplateProcess(srcFile, templateFile, includes, excludes)) {
				return;
		...
		private String proceeForOutputFilepath(Map<String,Object> filePathModel, String templateFile) throws IOException {
			...
			if ((testExpressionIndex = templateFile.indexOf('@')) != -1) {
				...
				if (!"true".equals(String.valueOf(expressionValue))) {
					logger.info("[not-generate]\t test expression '@" + testExpressionKey + "' is false,template:"
							+ templateFile);
					return null;
				}
			}
			...
			Configuration conf = GeneratorUtils.newFreeMarkerConfiguration(templateRootDirs, sourceEncoding,
					"/filepath/processor/");
		...
		private Template getFreeMarkerTemplate(String templateName) throws IOException {
			return GeneratorUtils.newFreeMarkerConfiguration(templateRootDirs, sourceEncoding, templateName)
					.getTemplate(templateName);
		}

		private void generateNewFileOrInsertIntoFile(String templateFile, String outputFilepath, Map<String, Object> templateModel)
				throws Exception {
			Template template = getFreeMarkerTemplate(templateFile);
