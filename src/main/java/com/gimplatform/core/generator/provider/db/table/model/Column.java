...
public class Column {
    ...
    public Column(Column c) {
        this(c.getTable(), c.getSqlType(), c.getSqlTypeName(), c.getColumnName(), c.getSize(), c.getDecimalDigits(), c.isPk(), c.isNullable(), c.isIndexed(), c.isUnique(), c.getDefaultValue(), c.getRemarks());
    }
    ...
    public void initPageProperties(JSONObject json) {
        if (json == null)
            return;
        if ("N".equals(json.getString("isDisplay")))
    ...
    /**
     * 列的别名，等价于：getRemarks().isEmpty() ? getColumnNameFirstLower() : getRemarks() 示例值: birthDate
     */
    ...
    public String getJapAnnotation() {
        ...
        if (_isPk) {
            ...
            if (_sqlTypeName.equals("DECIMAL") || _sqlTypeName.equals("INT")) {
                ...
                sb.append("\r\n\t");
                sb.append("@TableGenerator(name = \"" + _table.getClassName() + "IdGenerator\", table = \"sys_tb_generator\", " + "pkColumnName = \"GEN_NAME\", valueColumnName = \"GEN_VALUE\", pkColumnValue = \"" + _table.getSqlName().toUpperCase()
                        + "_PK\", allocationSize = 1)");
                sb.append("\r\n\t");
            }
        }
        // 处理时间类型
        if (_sqlTypeName.equals("DATETIME") || _sqlTypeName.equals("TIMESTAMP") || _sqlTypeName.equals("DATE") || _sqlTypeName.equals("TIME")) {
