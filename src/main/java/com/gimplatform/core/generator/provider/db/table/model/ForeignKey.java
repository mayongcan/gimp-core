...
public class ForeignKey {
	...
	private void initRelationship() {
		...
		List<Column> foreignPrimaryKeys = null;
		if(foreignTable != null) foreignPrimaryKeys = foreignTable.getPkColumns();
	...
	private boolean primaryKeyHasColumn(String aColumn) {
		...
		for (int i = 0; i < numKeys; i++) {
			Column sqlCol = (Column) parentTable.getPkColumns().get(i);
	...
	public static class ReferenceKey {
		...
		public ReferenceKey(String schemaName, String tableName, String columnSqlName) {
			this.schemaName = StringUtils.toString(schemaName, null);
