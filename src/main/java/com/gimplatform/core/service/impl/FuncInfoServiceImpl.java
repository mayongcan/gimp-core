...
public class FuncInfoServiceImpl implements FuncInfoService{
	...
	public boolean loadFuncDataToCache() {
		...
		if(allFuncList == null || allFuncList.size() == 0) return false;
		else{
			//权限列表写入内存中
...
//		return new ArrayList<FuncInfo>(set);
	...
	public JSONObject addFunc(FuncInfo funcInfo, UserInfo userInfo) {
		...
		if(funcInfo.getFuncType().equals(100200L)) funcInfo.setFuncLevel(1L);
		else if(funcInfo.getFuncType().equals(100300L)) funcInfo.setFuncLevel(2L);
		else if(funcInfo.getFuncType().equals(100400L)) funcInfo.setFuncLevel(3L);
	...
	public JSONObject editFunc(FuncInfo funcInfo, UserInfo userInfo) {
		...
		if(funcInfoInDb != null && !funcInfoInDb.getFuncFlag().equals(funcInfo.getFuncFlag()) && 
				funcInfoRepository.findByFuncFlagAndIsValid(funcInfo.getFuncFlag(), Constants.IS_VALID_VALID).size() > 0){
			return RestfulRetUtils.getErrorMsg("24005","权限标识已存在，请重新修改");
		}
		if(funcInfo.getFuncType().equals(100200L)) funcInfo.setFuncLevel(1L);
		else if(funcInfo.getFuncType().equals(100300L)) funcInfo.setFuncLevel(2L);
	...
	public JSONArray getJsonTree(List<Map<String, Object>> listFunc, String parentId){
		...
		if(parentId != null) root = new TreeNode(parentId, "all", null, false);
