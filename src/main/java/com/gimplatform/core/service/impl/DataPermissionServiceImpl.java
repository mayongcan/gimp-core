...
public class DataPermissionServiceImpl implements DataPermissionService {
    ...
    public JSONObject addDataPermission(DataPermission dataPermission, UserInfo userInfo) {
        if (dataPermission.getTenantsId() == null)
            dataPermission.setTenantsId(userInfo.getTenantsId());
        if (dataPermission.getOrganizerId() == null)
            dataPermission.setOrganizerId(userInfo.getOrganizerId());
        dataPermission.setIsValid(Constants.IS_VALID_VALID);
        if (dataPermissionRepository.findByPermissionNameAndTenantsIdAndOrganizerIdAndIsValid(dataPermission.getPermissionName(), dataPermission.getTenantsId(), dataPermission.getOrganizerId(), Constants.IS_VALID_VALID).size() > 0) {
    ...
    public JSONObject editDataPermission(DataPermission dataPermission, UserInfo userInfo) {
        ...
        if (dataPermissionInDb != null && !dataPermissionInDb.getPermissionName().equals(dataPermission.getPermissionName())
                && dataPermissionRepository.findByPermissionNameAndTenantsIdAndOrganizerIdAndIsValid(dataPermission.getPermissionName(), dataPermission.getTenantsId(), dataPermission.getOrganizerId(), Constants.IS_VALID_VALID).size() > 0) {
            return RestfulRetUtils.getErrorMsg("25006", "输入名称已存在");
        }
        // 合并两个javabean
    ...
    public Map<String, String> getTreeListByUser(UserInfo userInfo) {
        ...
        for (DataPermission obj : retList) {
            if (StringUtils.isBlank(firstId))
                firstId = obj.getPermissionId() + "";
            if (StringUtils.isBlank(firstName))
                firstName = obj.getPermissionName() + "";
